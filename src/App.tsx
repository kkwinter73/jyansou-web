...
const CPU_DELAY = 380; // CPU/自動進行の演出待ち(ms)
const SEAT_NAME = ['あなた', 'CPU右', 'CPU対面', 'CPU左'];
const newSeed = () => Math.floor(Math.random() * 2 ** 31);

function TileView({ tile, onClick, disabled, back }: {
  tile?: Tile;
  onClick?: () => void;
  disabled?: boolean;
  back?: boolean;
}) {
  if (back || !tile) return <span className="tile back" />;
  const cls = `tile ${tileSuitClass(tile.kind)}${tile.red ? ' red' : ''}${onClick ? ' clickable' : ''}${disabled ? ' disabled' : ''}`;
  return (
...
export function App() {
  ...
  // 自動進行: ツモ（全席）、CPUの打牌、CPUのロン/パス
  useEffect(() => {
    const s = game;
    if (final) return;
    let auto = false;
    if (s.phase === 'draw') auto = true;
    else if (s.phase === 'discard' && s.turn !== 0) auto = true;
    else if (s.phase === 'afterDiscard' && !s.pendingRon.includes(0)) auto = true;
    if (!auto) return;

    const timer = setTimeout(() => {
      setGame((g) => {
        if (g !== s) return g; // 古い状態には適用しない
        if (s.phase === 'draw') return apply(s, { type: 'draw' }).state;
        const seat = (s.phase === 'afterDiscard' ? s.pendingRon[0] : s.turn) as Seat;
        return apply(s, chooseAction(s, seat)).state;
      });
    }, CPU_DELAY);
  ...
  const canRon = game.phase === 'afterDiscard' && game.pendingRon.includes(0);
  ...
  const nextHand = () => {
    ...
    if (r.over) setFinal({ ranking: r.ranking });
  ...
  // ツモ牌は手番プレイヤーのもの。人間の手牌欄には自分の手番のときだけ分離表示する。
    ...
    <div className="app">
      <header>
        ...
        <div className="round">
          ...
          <span className="dora">ドラ表示<TileView tile={game.doraIndicators[0]} /></span>
        </div>
      </header>
      ...
      <section className="table">
        ...
        <div className="panel me">
          ...
          <div className="hand">
            {inHand.map((t) => (
              <TileView
                ...
                disabled={myTurn && riichiMode && !riichiKinds.has(t.kind)}
              />
            ))}
            {drawn && (
          ...
          <div className="controls">
            ...
            {canRon && (
              <>
                <button className="primary" onClick={() => act({ type: 'ron', seat: 0 })}>ロン</button>
                <button onClick={() => act({ type: 'pass', seat: 0 })}>パス</button>
              </>
            )}
            {!myTurn && !canRon && game.phase !== 'over' && <span className="hint">…</span>}
      ...
      {final && <FinalOverlay ranking={final.ranking} onRestart={restart} />}
    </div>
  );
}
    ...
    <div className="overlay">
      <div className="card">
        ...
        <ol className="ranking">
          {ranking.map((e) => (
            <li key={e.seat}>
              {e.rank}位 {SEAT_NAME[e.seat]} — {e.score}
            </li>
          ))}
        </ol>
